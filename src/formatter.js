...
export default class Formatter {
  ...
  format(value, indent = 0) {
    ...
    else if (_.isArray(value)) return this.formatArray(value, indent);
    else if (_.isObject(value)) return this.formatObject(value, indent);
  ...
  formatObject(object, indent) {
    ...
    if (
      this.compact &&
      _.keys(object).length === 1 &&
      _.every(object, this.isSimple)
    ) {
      let formatted = _.map(
        object,
        (value, key) => `"${key}": ` + this.format(value, indent + this.step)
      );
  ...
  formatArray(array, indent) {
    ...
    return (
      "[\n" +
      _.join(
        _.map(array, (x) => spaces + this.format(x, indent + this.step)),
        ",\n"
      ) +
      "\n" +
      _.repeat(" ", indent) +
